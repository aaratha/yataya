...
function Destinations({ destinations, setDestinations }) {
    ...
    const submitDestinations = () => {
        if (destinations.length === 0) {
            setSubmitStatus({ type: "error", message: "Add at least one destination first." });
            return;
        }

        setSubmitStatus({ type: "pending", message: "Saving destinations..." });
        fetch("http://localhost:8080/api/travel/set-destinations", {
            method: "POST",
            ...
            .then((res) => {
                ...
                setSubmitStatus({ type: "success", message: "Destinations saved successfully." });
            })
            .catch(() => {
                setSubmitStatus({ type: "error", message: "Could not save destinations." });
            });
    };
